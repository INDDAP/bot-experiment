...
# --- Boilerplate Code (Logging, Flask, API, State, Dashboard) ---
...
def run_flask_app(): logging.getLogger('werkzeug').setLevel(logging.ERROR); app.run(host='0.0.0.0', port=8084) # New Port
...
def check_signal(df):
    ...
    # 2. Volume Spike
    volume_confirms = latest['volume'] > latest['volume_avg'] * VOLUME_SPIKE_FACTOR
    # 3. ADX Rising
    adx_is_strong = latest[f'ADX_{ADX_PERIOD}'] > ADX_THRESHOLD
    adx_is_rising = latest[f'ADX_{ADX_PERIOD}'] > prev[f'ADX_{ADX_PERIOD}']
    
    if ema_cross_up and volume_confirms and adx_is_strong and adx_is_rising:
        msg = f"📈 Long Signal Found! Volume Spike & ADX Rising."
        ...
        try:
            while True:
                ...
                if not trade_manager.position_open:
                    msg = f"Searching... Price: ${current_price:<9.2f} | ADX: {candles_df.iloc[-1][f'ADX_{ADX_PERIOD}']:.2f}"
