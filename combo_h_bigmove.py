...
# TP=1.5%, SL=0.7%. We will risk 0.5% of the balance per trade
...
def run_flask_app(): logging.getLogger('werkzeug').setLevel(logging.ERROR); app.run(host='0.0.0.0', port=8086) # New Port
...
def add_indicators(df):
    ...
    df['swing_high'] = df['high'].rolling(window=SWING_PERIOD).max()
    df['swing_low'] = df['low'].rolling(window=SWING_PERIOD).min()
    return df

def check_signal(df):
    latest = df.iloc[-1]; prev = df.iloc[-2]
    # Trend Filter
    ema_trend = latest[f'EMA_{EMA_FAST}'] > latest[f'EMA_{EMA_SLOW}']
    # Volatility Filter
    atr_value = latest[f'ATR_{ATR_PERIOD}']
    volatility_check = atr_value >= (ATR_THRESHOLD_PERCENT / 100) * latest['close']

    # Long conditions
    long_entry_rsi = prev[f'RSI_{RSI_PERIOD}'] < RSI_OVERSOLD and latest[f'RSI_{RSI_PERIOD}'] > RSI_OVERSOLD + 5 # >35
    long_adx = latest[f'ADX_{ADX_PERIOD}'] > ADX_THRESHOLD
    long_breakout = latest['close'] > latest['swing_high']
    long_volume = latest['volume'] > latest['volume_avg'] * VOLUME_SPIKE_FACTOR

    # Short conditions
    short_entry_rsi = prev[f'RSI_{RSI_PERIOD}'] > RSI_OVERBOUGHT and latest[f'RSI_{RSI_PERIOD}'] < RSI_OVERBOUGHT - 5 # <65
    short_adx = latest[f'ADX_{ADX_PERIOD}'] > ADX_THRESHOLD
    short_breakout = latest['close'] < latest['swing_low']
    short_volume = latest['volume'] > latest['volume_avg'] * VOLUME_SPIKE_FACTOR

    if ema_trend and long_entry_rsi and long_adx and long_breakout and long_volume and volatility_check:
        msg = "📈 Long Signal Found: Big Move Catcher!"
        logger.info(msg); system_log.append(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")
        return "long"

    elif not ema_trend and short_entry_rsi and short_adx and short_breakout and short_volume and volatility_check:
        msg = "📉 Short Signal Found: Big Move Catcher!"
        logger.info(msg); system_log.append(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")
        return "short"
    ...
    with Live(generate_dashboard(trade_manager, current_price), screen=True, redirect_stderr=False, refresh_per_second=4) as live:
        try:
            while True:
                ...
                if candles_df is None or candles_df.empty: await asyncio.sleep(60); continue
                ...
                if not trade_manager.position_open:
                    ...
                    if signal:
                        ...
                        risk_amount_usdt = trade_manager.balance * (RISK_PER_TRADE_PERCENT / 100)
                        stop_loss_abs_price = entry_price * (1 - SL_PERCENT / 100) if signal == "long" else entry_price * (1 + SL_PERCENT / 100)
                        stop_loss_pips = abs(entry_price - stop_loss_abs_price)
                        qty = risk_amount_usdt / stop_loss_pips if stop_loss_pips > 0 else 0
                        if qty > 0 and trade_manager.balance > (qty * entry_price):
                            trade_manager.position_side = signal
                            trade_manager.qty = qty
                            trade_manager.stop_loss_price = stop_loss_abs_price
                            trade_manager.take_profit_price = entry_price * (1 + TP_PERCENT / 100) if signal == "long" else entry_price * (1 - TP_PERCENT / 100)
                            trade_manager.entry_price = entry_price
                            trade_manager.position_open = True
