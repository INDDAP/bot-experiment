...
BOT_NAME = "Custom_SqueezeBreakout"
STATE_FILE = f"state_{BOT_NAME}.json"
LOG_FILE = f"log_{BOT_NAME}.txt"
START_BALANCE = 1000.0
# We will risk a tiny amount per trade to make up for the tight SL
...
BB_LENGTH = 20
BB_STDEV = 2
BB_SQUEEZE_THRESHOLD = 0.05 # Smaller number = tighter squeeze
TP_PERCENT = 2.5 # High initial TP to capture big moves
SL_PERCENT = 0.5 # Very tight SL
TRAILING_STOP_PERCENT = 0.5 # Trailing stop will be activated once profit exceeds this %
...
def run_flask_app(): logging.getLogger('werkzeug').setLevel(logging.ERROR); app.run(host='0.0.0.0', port=8089)
...
class TradeManager:
    ...
    def update_trailing_stop(self, current_price, trailing_stop_percent):
        if self.position_side == 'long':
            ...
            if profit_pct > trailing_stop_percent:
                new_stop = current_price * (1 - trailing_stop_percent / 100)
                if new_stop > self.stop_loss_price: self.stop_loss_price = new_stop
...
def check_signal(df):
    ...
    # Check for a Bollinger Band squeeze
    bandwidth = (latest['BBU_20_2.0'] - latest['BBL_20_2.0']) / latest['BBM_20_2.0']
    is_squeezing = bandwidth < BB_SQUEEZE_THRESHOLD
    
    # Check for a breakout after the squeeze
    if is_squeezing: return None
    
    # Breakout from below (long signal)
    if prev['BBU_20_2.0'] < prev['close'] and latest['BBU_20_2.0'] < latest['close']:
        return "long"
        
    # Breakout from above (short signal)
    if prev['BBL_20_2.0'] > prev['close'] and latest['BBL_20_2.0'] > latest['close']:
        return "short"
    ...
    with Live(generate_dashboard(trade_manager, current_price), screen=True, redirect_stderr=False, refresh_per_second=4) as live:
        try:
            while True:
                ...
                if not trade_manager.position_open:
                    ...
                    if signal:
                        ...
                        if qty > 0 and trade_manager.balance > (qty * entry_price):
                            trade_manager.position_side = signal; trade_manager.qty = qty
                ...
                else: # Check Exit Logic & Trailing Stop
